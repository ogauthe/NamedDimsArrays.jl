...
using NamedDimsArrays: NamedDimsArrays, @names, AbstractNamedDimsArray,
    AbstractNamedDimsMatrix, NaiveOrderedSet, Name, NameMismatch, NamedDimsArray,
    NamedDimsCartesianIndex, NamedDimsCartesianIndices, NamedDimsMatrix, aligndims,
    aligneddims, apply, dename, denamed, dim, dimnames, dims, fusednames, isnamed,
    mapnameddimsindices, name, named, nameddimsarray, nameddimsindices, namedoneto,
    operator, product, replacenameddimsindices, setnameddimsindices, state, unname, unnamed
...
@testset "NamedDimsArrays.jl" begin
    @testset "Basic functionality (eltype=$elt)" for elt in elts
        ...
        @test_throws ErrorException NamedDimsArray(
            randn(4),
            namedoneto.((2, 2), ("i", "j"))
        )
        @test_throws ErrorException NamedDimsArray(
