...
using TensorAlgebra: TensorAlgebra, blockedperm, contract, contract!, contractadd!, eigen,
    eigvals, factorize, left_null, left_orth, left_polar, lq, matricize, orth, permmortar,
    polar, qr, right_null, right_orth, right_polar, svd, svdvals, unmatricize
...
function TensorAlgebra.matricize(na::AbstractNamedDimsArray, fusions::Vararg{Pair, 2})
    ...
    if sum(length, nameddimsindices_fuse) < ndims(na)
        ...
        nameddimsindices_unspecified =
            setdiff(nameddimsindices(na), nameddimsindices_fuse...)
...
for f in [
    ...
    @eval begin
        function TensorAlgebra.$f(
            ...
            x_unnamed, y_unnamed =
                $f(dename(a), nameddimsindices(a), codomain, domain; kwargs...)
...
function TensorAlgebra.eigen(
    ...
    domain = to_nameddimsindices(a, dimnames_domain)
    d_unnamed, v_unnamed =
        eigen(dename(a), nameddimsindices(a), codomain, domain; kwargs...)
